...
docker run -m 64GB -d --name $container -e "JAVA_TOOL_OPTIONS=-Xmx50g" --mount type=bind,source=/srv/data/iq26og/dbpedia/,target=/app/data --mount type=bind,source=/srv/data/iq26og/git/qse/,target=/app/local $image /app/local/config/wd-max-card/dbpediaConfig.properties
