...
container=qse_container_lubm

echo "About to run docker container: ${container}"

docker run -m 16GB -d --name $container -e "JAVA_TOOL_OPTIONS=-Xmx10g" --mount type=bind,source=/data/,target=/app/data --mount type=bind,source=/qse/,target=/app/local $image /app/local/config/lubmConfig.properties
