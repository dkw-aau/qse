...
public class BaselineWithART {
    ...
    NavigableMap<String, List<Node>> art = new AdaptiveRadixTree<>(BinaryComparables.forString());
    
    // Constructor
    BaselineWithART(String filePath) {
        this.rdfFile = filePath;
    }
    
    public BaselineWithART(String filePath, int expSizeOfClasses) {
        this.rdfFile = filePath;
                    ...
                    .forEach(line -> {                              // - A terminal operation
                        try {
                            ...
                            if(art.containsKey(nodes[0].getLabel())){
                                art.get(nodes[0].getLabel()).add(nodes[2]);
                            } else {
                                List<Node> list = new ArrayList<>();
                                list.add(nodes[2]);
                                art.put(nodes[0].getLabel(), list);
                            }
            ...
            Files.lines(Path.of(rdfFile))                           // - Stream of lines ~ Stream <String>
                    ...
                    .forEach(line -> {                              // - A terminal operation
                        try {
                            Node[] nodes = NxParser.parseNodes(line);
                            if (art.containsKey(nodes[0].getLabel())) {
                                art.get(nodes[0].getLabel()).forEach(c -> {
                                    if (classToPropWithObjTypes.containsKey(c)) {
                                        ...
                                        if (art.containsKey(nodes[2].getLabel())) // object is an instance of some class e.g., :Paris is an instance of :City.
                                            art.get(nodes[2].getLabel()).forEach(node -> {
                                        ...
                                        art.get(nodes[0].getLabel()).forEach(cl -> {
